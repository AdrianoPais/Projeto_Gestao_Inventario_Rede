...
with st.sidebar:
    ...
    if lista_dicts:
        ...
        # 2. CSV e Excel
        c_exp = st.columns(2)
        c_exp[0].download_button("📊 CSV", data=df.to_csv(index=False).encode('utf-8'), file_name="inventario.csv", key="btn_csv")
        
        buf = BytesIO()
        with pd.ExcelWriter(buf, engine='openpyxl') as wr:
            df.to_excel(wr, index=False, sheet_name='Dispositivos')
        c_exp[1].download_button("📗 Excel", data=buf.getvalue(), file_name="inventario.xlsx", key="btn_excel")

        # 3. PDF Oficial
        pdf_data = gerar_pdf(inv.list_devices())
        st.download_button("📕 PDF Oficial", data=pdf_data, file_name="inventario_oficial.pdf", key="btn_pdf")

        # 4. TXT DETALHADO (Recuperado)
